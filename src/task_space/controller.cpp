  ...
  namespace task_space {
    ...
    Eigen::VectorXd CompliantController::computeEffort(RobotState const& desired_state,
      ...
      try {
        ...
        ROS_ERROR_THROTTLE(
            1.0,
            "[compliant_controllers/task_space] Pinocchio "
      ...
      if (!gravity_.allFinite()) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[compliant_controllers/task_space] Pinocchio gravity "
      ...
      if (!nominal_theta_.allFinite() || !nominal_theta_dot_.allFinite()) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[compliant_controllers/task_space] integrator state went "
            "non-finite — resetting nominal_theta_*_prev_ to current");
                                                                  ...
                                                                  Eigen::VectorXd const& current_q)
    {
      ...
      if (!q_error_sum_.allFinite()) {
        ...
        ROS_WARN_THROTTLE(
            1.0,
            "[compliant_controllers/task_space] q_error_sum_ went "
